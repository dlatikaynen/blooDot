...
typedef struct _MESHVERTEXELEMENT
{
...
} MESHVERTEXELEMENT, *LPMESHVERTEXELEMENT;

struct MESH_VERTEX_BUFFER_HEADER
{
    UINT64 NumVertices;
    UINT64 SizeBytes;
    UINT64 StrideBytes;
    MESHVERTEXELEMENT Decl[MAX_VERTEX_ELEMENTS];
    union
    {
...
struct MESH_INDEX_BUFFER_HEADER
{
    UINT64 NumIndices;
    UINT64 SizeBytes;
    UINT32 IndexType;
...
//--------------------------------------------------------------------------------------
// Mesh class.  This class reads the mesh file format
//--------------------------------------------------------------------------------------
class Mesh
{
private:
...
protected:
    ...
    MESH_VERTEX_BUFFER_HEADER* m_vertexBufferArray;
    MESH_INDEX_BUFFER_HEADER* m_indexBufferArray;
    MESH_MESH* m_meshArray;
    MESH_SUBSET* m_subsetArray;
    MESH_FRAME* m_frameArray;
    MESH_MATERIAL* m_materialArray;

    // Adjacency information (not part of the m_pStaticMeshData, so it must be created and destroyed separately)
    MESH_INDEX_BUFFER_HEADER* m_adjacencyIndexBufferArray;

    // Animation
    ANIMATION_FILE_HEADER* m_animationHeader;
    ANIMATION_FRAME_DATA* m_animationFrameData;
    XMMATRIX* m_bindPoseFrameMatrices;
    XMMATRIX* m_transformedFrameMatrices;
    XMMATRIX* m_worldPoseFrameMatrices;

protected:
    void LoadMaterials(ID3D11Device* d3dDevice, _In_reads_(numMaterials) MESH_MATERIAL* materials, uint32 numMaterials);
    HRESULT CreateVertexBuffer(ID3D11Device* d3dDevice, MESH_VERTEX_BUFFER_HEADER* header, void* vertices);
    HRESULT CreateIndexBuffer(ID3D11Device* d3dDevice, MESH_INDEX_BUFFER_HEADER* header, void* indices);
...
public:
    ...
    // Helpers
    static D3D11_PRIMITIVE_TOPOLOGY GetPrimitiveType(MeshPrimitiveType primitiveType);
    ...
    MeshIndexType GetIndexType(uint32 mesh);
    ...
    MESH_MATERIAL* GetMaterial(uint32 material);
    MESH_MESH* GetMesh(uint32 mesh);
    uint32 GetNumSubsets(uint32 mesh);
    MESH_SUBSET* GetSubset(uint32 mesh, uint32 subset);
    uint32 GetVertexStride(uint32 mesh, uint32 vertexBuffer);
    uint32 GetNumFrames();
    MESH_FRAME* GetFrame(uint32 frame);
    MESH_FRAME* FindFrame(char* name);
