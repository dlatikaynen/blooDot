...
namespace blooDot
{
	namespace Orchestrator
	{
		...
		void EnvelopeSource::Disable()
		{
			...
			{
				this->m_Release = this->m_Value / (this->m_releaseDuration * OrchestratorBase::SampleRate());
			}
		}
		...
		void EnvelopeSource::SetAttackDuration(double attack)
		{
			...
			this->m_Attack = 1. / (attack * OrchestratorBase::SampleRate());
		}
		...
		void EnvelopeSource::SetReleaseDuration(double release)
		{
			...
			this->m_Release = this->m_sustainLevel / (release * OrchestratorBase::SampleRate());
		...
		void EnvelopeSource::SetDecayDuration(double decay)
		{
			...
			this->m_Decay = (1. - this->m_sustainLevel) / (decay * OrchestratorBase::SampleRate());
		}
		...
		inline AudioFrames& EnvelopeSource::Render(AudioFrames& frames, unsigned int channel)
		{
