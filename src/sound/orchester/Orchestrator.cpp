...
namespace blooDot
{
	...
	{
		Orchestrator::Orchestrator(double decayTime)
		{
			this->m_lastFrame.Resize(1, 1, 0.0);
			this->m_muteTime = (int)(decayTime * OrchestratorBase::SampleRate());
			this->m_Tags = INITIAL_TAGNUMBER;
		}

		void Orchestrator::AddSynthesizer(Synthesizer *synth, int group)
		{
			Orchestrator::SynthesizerSource synthSource;
		...
		void Orchestrator::RemoveSynthesizer(Synthesizer *synth)
		{
			bool found = false;
			std::vector<Orchestrator::SynthesizerSource>::iterator i;
		...
		long Orchestrator::Start(double midiNoteNumber, double amplitude, int group)
		{
			unsigned int i;
		...
		void Orchestrator::Stop(double noteNumber, double amplitude, int group)
		{
		...
		void Orchestrator::Stop(long tag, double amplitude)
		{
			for (unsigned int i = 0; i < this->m_synthesizerSources.size(); ++i)
			{
		...
		void Orchestrator::SetFreq(double noteNumber, int group)
		{
		...
		void Orchestrator::SetFreq(long tag, double noteNumber)
		{
			auto freq = static_cast<double>(A3_OFFSET) * ::pow(2.0, (noteNumber - MIDINOTE_MAX) / HALFNOTES_PER_OCTAVE);
			for (unsigned int i = 0; i < this->m_synthesizerSources.size(); i++)
			{
		...
		void Orchestrator::Bend(double value, int group)
		{
			double factor;
		...
		void Orchestrator::Bend(long tag, double value)
		{
			double factor;
			if (value < BEND_MIDPOINT)
			{
		...
		void Orchestrator::SetCtrl(int number, double value, int group)
		{
			for (unsigned int i = 0; i < this->m_synthesizerSources.size(); ++i)
			{
				if (this->m_synthesizerSources[i].group == group)
		...
		void Orchestrator::SetCtrl(long tag, int number, double value)
		{
			for (unsigned int i = 0; i < this->m_synthesizerSources.size(); ++i)
			{
				if (this->m_synthesizerSources[i].tag == tag)
