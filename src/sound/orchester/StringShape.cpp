...
namespace blooDot
{
	namespace Orchestrator
	{
		StringShape::StringShape(double minFreq)
		{
			...
			auto delay = static_cast<unsigned long>(OrchestratorBase::SampleRate() / minFreq);
		...
		{
			this->m_mrFreq = freq;
			this->m_mrLen = OrchestratorBase::SampleRate() / this->m_mrFreq;
		...
		void StringShape::SetTension(double tension)
		{
			...
			double dFreq = ((.5 * OrchestratorBase::SampleRate()) - freq) * .25;
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				coefficient = -2.0 * temp * ::cos(Orchestrator::PI_2 * freq / OrchestratorBase::SampleRate());
		...
		{
			auto normalizedValue = value * Orchestrator::INVERSE_HALFBYTE;
		...
		double StringShape::Render(unsigned int channel)
		{
		...
		AudioFrames& StringShape::Render(AudioFrames& frames, unsigned int channel)
		{
