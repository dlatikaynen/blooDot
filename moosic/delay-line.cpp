...
namespace blooDot::Oscillator
{
	...
	double DelayLine::GetOutputAt(unsigned long atTime)
	{
	...
	void DelayLine::SetOutputAt(double value, unsigned long atTime)
	{
		auto index = this->m_inputIndex - atTime - 1;
		while (index < 0)
		{
	...
	double DelayLine::GetNextOutput(void)
	{
	...
	double DelayLine::GetDelay(void) const
	{
		return this->m_Delay;
	}

	unsigned long DelayLine::GetMaxDelay(void)
	{
		return static_cast<unsigned long>(this->m_Inputs.Size()) - 1;
	}

	double DelayLine::Render(double input)
	{
	...
	AudioFrame& DelayLine::Render(AudioFrame& frames, unsigned int channel)
	{
	...
	double DelayLine::GetLastOutput(void) const
	{
		return this->m_lastFrame[0];
	}
}